...
def dealwithdata(protein):
    ...
    probMatrs = np.array(probMatrs)[indexes]
    NDs = np.array(NDs)[indexes]
    DPCPs = np.array(DPCPs)[indexes]
    Kmers = np.array(Kmers)[indexes]
    seq = np.array(seq)[indexes]
    dataY = np.array(dataY)[indexes]
    adj_matrices = np.array(adj_matrices)[indexes]  # 按相同的索引打乱邻接矩阵
    train_X, test_X,train_seq, test_seq,train_adj, test_adj, train_y, test_y = train_test_split(dataX,seq,adj_matrices, dataY, test_size=0.2)
    ...
    return train_X, test_X, train_seq, test_seq,train_adj, test_adj, train_y, test_y, embedding_matrix
